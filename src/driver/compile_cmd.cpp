...
namespace scl {
auto RunCompileCmd(const CommandLineOptions& options, const DriverEnv& env)
    -> CompileResult {
    ...
    if (file.getError()) {
        env.error_stream << llvm::formatv("error opening file {0} for read", options.input_filenames[0])
                     << '\n';
        return false;
    }
